...
class Solution( object ):
    ...
    def _calculate_dpdy_0(self, yPosition):
        Mu = self.M + self.alpha*self.alpha*self.Q
        p_0 = self.alpha*self.Q*(self.tao_0 + 0.5*self.rho*self.g*self.height)/Mu - self.rho_f*self.g
        return p_0
    ...
    def _calculate_dvdy_0(self, yPosition):
        Mu = self.M + self.alpha*self.alpha*self.Q
        v_0 = (self.rho - self.alpha*self.rho_f)*self.g*(yPosition - self.height/2.)/self.M \
              - (self.tao_0 + 0.5*self.rho*self.g*self.height) / Mu
        return v_0
    ...
    def getPressureValue( self, yPosition, time, numberOfSummationTerms = 200 ):
        if time == 0.0:
            ...
            for j in range(1, numberOfSummationTerms):
                term_1 = (-1.0)**(j-1) / (2.0*j - 1.0)
                term_2 = math.exp( -(time*self.c*(math.pi**2)*(2.0*j - 1.0)**2.0 ) / (4.0*self.height**2.0) )
                term_3 = math.cos( (2.0*j - 1.0)*math.pi*yPosition/(2.0*self.height) )
                summationResult += term_1*term_2*term_3
            barP0 = self._calculate_p_0(self.height)
            pressureValue = self.rho_f*self.g*(self.height - yPosition) + 4.0*barP0*summationResult/math.pi
            return pressureValue

    def getPressureGradientValue( self, yPosition, time, numberOfSummationTerms = 200 ):
        position = yPosition;
        if time == 0.0:
            pressureValue = self._calculate_dpdy_0(yPosition);
            return pressureValue
        else:
            summationResult = 0
            for j in range(1, numberOfSummationTerms):
                term_1 = (-1.0)**(j-1) / (2.0*j - 1.0)
                term_2 = math.exp( -(time*self.c*(math.pi**2)*(2.0*j - 1.0)**2.0 ) / (4.0*self.height**2.0) )
                term_3 = -(2.0*j - 1.0)*math.pi*math.sin( (2.0*j - 1.0)*math.pi*position/(2.0*self.height) )/(2.0*self.height)
                summationResult += term_1*term_2*term_3
            barP0 = self._calculate_p_0(self.height)
            pressureValue = -self.rho_f*self.g + 4.0*barP0*summationResult/math.pi
            return pressureValue
    ...
    def getDisplacementValue(self, yPosition, time, numberOfSummationTerms = 200):
        if time == 0.0:
            ...
            summationResult = 0;
            for j in range(1, numberOfSummationTerms):
                term_1 = ((-1.0)**(j+1))/((2.0*j - 1.0)**2)
                term_2 = math.exp( -((2.0*j - 1.0)**2.0)*(math.pi**2.0)*self.c*time/(4.0*self.height**2.0) )
                term_3 = math.sin((math.pi*yPosition*(2.0*j - 1.0))/(2*self.height))
                summationResult += term_1*term_2*term_3
            barP0 = self._calculate_p_0(self.height)
            ...
            displacementValue += 8.0*self.alpha*self.height*barP0*summationResult/(math.pi*math.pi*self.M)
            return displacementValue;

    def getDisplacementGradientValue(self, yPosition, time, numberOfSummationTerms=200, modulus=1.):
        if time == 0.0:
            dvdz = self._calculate_dvdy_0(yPosition);
        else:
            summationResult = 0;
            for j in range(1, numberOfSummationTerms):
                term_1 = ((-1.0)**(j+1))/((2.0*j - 1.0)**2)
                term_2 = math.exp( -((2.0*j - 1.0)**2.0)*(math.pi**2.0)*self.c*time/(4.0*self.height**2.0) )
                term_3 = (2.0*j - 1.0)*math.cos((math.pi*yPosition*(2.0*j - 1.0))/(2*self.height))
                summationResult += term_1*term_2*term_3
            barP0 = self._calculate_p_0(self.height)
            dvdz = -self.tao_0/self.M
            dvdz += -(self.g/self.M)*(self.rho - self.alpha*self.rho_f)*(self.height - yPosition)
            dvdz += 4.0*self.alpha*barP0*summationResult/(math.pi*self.M)
        return modulus*dvdz;

    def getEffectiveStressValue(self, yPosition, time, numberOfSummationTerms=200):
        return self.getDisplacementGradientValue(yPosition, time, numberOfSummationTerms, self.M)

    def getTotalStressValue(self, yPosition, time, numberOfSummationTerms=200):
        s = self.getEffectiveStressValue(yPosition, time, numberOfSummationTerms)
        p = self.getPressureValue(yPosition, time, numberOfSummationTerms)
        return s - self.alpha*p + self.rho*self.g*(self.height - yPosition)
    ...
    def getPressureValuesConstTime(self, time, numberOfSummationTerms=200, ny=200):
        ...
        pressureValues = np.zeros(size);
        for i in range(0, size):
            pressureValues[i] = self.getPressureValue(positionValues[i], time, numberOfSummationTerms)
        return pressureValues

    def getPressureGradientValuesConstTime(self, time, numberOfSummationTerms=200, ny=200):
        positionValues, size = self.__getPositionValuesAndSize(ny)
        dpdy = np.zeros(size);
        for i in range(0, size):
            dpdy[i] = self.getPressureGradientValue(positionValues[i], time, numberOfSummationTerms)
        return dpdy
    ...
    def getDisplacementValuesConstTime( self, time, numberOfSummationTerms = 200, ny = 200 ):
        ...
        positionValues, size = self.__getPositionValuesAndSize(ny)
        displacementValues = np.zeros(size)
        for i in range(0, size):
            displacementValues[i] = self.getDisplacementValue( positionValues[ i ], time, numberOfSummationTerms );
        return displacementValues;

    def getDisplacementGradientValuesConstTime(self, time, numberOfSummationTerms=200, ny=200, modulus=1.):
        positionValues, size = self.__getPositionValuesAndSize(ny)
        dwdzValues = np.zeros(size)
        for i in range(0, size):
            dwdzValues[i] = self.getDisplacementGradientValue(positionValues[i], time, numberOfSummationTerms, modulus);
        return dwdzValues;

    def getEffectiveStressValuesConstTime(self, time, numberOfSummationTerms=200, ny=200):
        return self.getDisplacementGradientValuesConstTime(time, numberOfSummationTerms, ny, self.M)

    def getTotalStressValuesConstTime(self, time, numberOfSummationTerms=200, ny=200):
        positionValues, size = self.__getPositionValuesAndSize(ny)
        s = self.getEffectiveStressValuesConstTime(time, numberOfSummationTerms, positionValues)
        p = self.getPressureValuesConstTime(time, numberOfSummationTerms, positionValues)
        return s - self.alpha*p + self.rho*self.g*(self.height - positionValues)
